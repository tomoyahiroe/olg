...
@njit
def update_distribution_period_numba(mea_current, mea_next, opt_indexes_t, 
                                    a_grid, aprime_grid, P, NJ, Nl, Na):
    """
    1期間の分布更新（numba最適化版）
    """
    # 0歳の家計：資産0、スキル50%ずつ
...
def solve_forward_transition(tr_setting, setting, opt_indexes, mu_ini):
    ...
    for t in range(1, tr_setting.NT):
        ...
        update_distribution_period_numba(
            mea_current, mea_next, opt_indexes[t-1],
            setting.a_grid, setting.aprime_grid, setting.P,
