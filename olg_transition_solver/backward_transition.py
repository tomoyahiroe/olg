...
@njit
def solve_household_age_numba(h, NJ, Nl, Na, Naprime, setting_params, 
                             value_fun_next, r_t, y_h, 
        ...
        for i_l in range(Nl):
            for i_a in range(Na):
                ...
                for j_a in range(Naprime):
                    ...
                    _, ac1vec, ac2vec, weight_1, weight_2 = inverse_interp_aprime_point_numba(a_grid, aprime)
    ...
    for t in range(tr_setting.NT - 1, -1, -1):
        ...
        for h in range(setting.NJ):
            for i_l in range(setting.Nl):
                y_matrix[h, i_l] = calculate_income_numba(h, i_l, r_t, w_t, tau_t, p_t, 
                                                         setting.l_grid, setting.Njw)
        
        # 当期の価値関数・政策関数の箱
        vfun_current = np.zeros((setting.NJ, setting.Nl, setting.Na))
        afun_current = np.zeros((setting.NJ, setting.Nl, setting.Na))
        afun_index_current = np.zeros((setting.NJ, setting.Nl, setting.Na), dtype=np.int32)
        
        # 各年齢の最適化を実行
        for h in range(setting.NJ-1, -1, -1):  # 最終年齢から逆順
            solve_household_age_numba(
                h, setting.NJ, setting.Nl, setting.Na, setting.Naprime,
                setting_params, value_fun_box, r_t, y_matrix[h],
                vfun_current, afun_current, afun_index_current
            )
