...
from olg.transition import (
    run_transition_analysis,
)


def save_results_to_pkl(results, prefix="results", timestamp=False):
    """
    結果をpickleファイルとして保存
    ...
    if timestamp:
        ...
        output_dir = f"{prefix}_{timestamp_str}"
    else:
        output_dir = f"{prefix}"
...
if __name__ == "__main__":
    ...
    print("\n=== カスタム設定結果の保存 ===")
    saved_files = save_results_to_pkl(results, prefix="psi_50to75_by25T")
